...
// searchGrounding: google_search ツール有効化フラグ
$searchGrounding = isset($input['searchGrounding']) && $input['searchGrounding'] === true;
$temperature = isset($input['temperature']) && is_numeric($input['temperature'])
    ? floatval($input['temperature']) : 0.7;

// デバッグ: リクエスト情報をログ
$sgLabel = $searchGrounding ? 'ON' : 'OFF';
@file_put_contents(__DIR__ . '/debug.log',
    date('Y-m-d H:i:s') . " [INFO] model={$modelId} messages=" . count($messages) .
    " rawInputLen=" . strlen($rawInput) . " searchGrounding={$sgLabel}\n",
    FILE_APPEND | LOCK_EX);
...
if (strpos($modelId, 'gpt-') === 0) {
    ...
    handleGemini($modelId, $messages, $searchGrounding, $temperature);
}
...
function isGeminiSupported($mime) {
    ...
    // PDF は Gemini API が inlineData でネイティブ対応
...
// ========== Gemini ==========
function handleGemini($modelId, $messages, $searchGrounding = false, $temperature = 0.7) {
    ...
    curl_setopt($ch, CURLOPT_WRITEFUNCTION, function ($ch, $data) use (&$outputSent, &$errorBuffer, &$lineBuffer, $searchGrounding) {
        $lineBuffer .= $data;
        // 完結した行（\n で終わるもの）だけ処理、未完結は次チャンクへ持ち越し
        while (($pos = strpos($lineBuffer, "\n")) !== false) {
            $line = trim(substr($lineBuffer, 0, $pos));
            ...
            if (strpos($line, 'data: ') === 0) {
                ...
                // 検索グラウンディング: 引用元URL抽出
                if ($searchGrounding && isset($json['candidates'][0]['groundingMetadata']['groundingChunks'])) {
                    $sources = [];
                    foreach ($json['candidates'][0]['groundingMetadata']['groundingChunks'] as $chunk) {
                    ...
                    if (!empty($sources)) {
                        echo json_encode(['sources' => $sources], JSON_UNESCAPED_UNICODE) . "\n";
                        @flush();
                    }
                }
            } else {
