<?php
/**
 * app12.php - 骨太の方針2026 チャット相談（Dify SSE プロキシ）
 * 「経済財政運営と改革の基本方針2026（原案）」のナレッジベースを持つ Dify アプリに接続し、
...
set_error_handler(function($errno, $errstr, $errfile, $errline) {
    @file_put_contents(__DIR__ . '/debug.log',
        date('Y-m-d H:i:s') . " [app12] [{$errno}] {$errstr} in {$errfile}:{$errline}\n",
        FILE_APPEND | LOCK_EX);
    return true;
});
error_reporting(0);
...
if (is_file($envFile)) {
    foreach (@file($envFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) as $ln) {
        ...
        $k = trim($p[0]);
        $v = trim($p[1]);
        if ($k === 'DIFY_HONEBUTO_KEY') $apiKey = $v;
    }
}

if (!$apiKey) {
    emitError('DIFY_HONEBUTO_KEY が設定されていません');
    exit;
}
...
@file_put_contents(__DIR__ . '/debug.log',
    date('Y-m-d H:i:s') . " [app12] query=" . mb_substr($query, 0, 80) .
